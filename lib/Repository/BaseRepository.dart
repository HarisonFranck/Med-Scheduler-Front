...
class BaseRepository {
  ...
  Future<void> patchAppointment(CustomAppointment appointment) async {
    if (await utilities.isConnectionAvailable()) {
      authProvider = Provider.of<AuthProvider>(context, listen: false);
      token = authProvider.token;

      final url = Uri.parse(
          "${baseUrl}api/appointments/${utilities.extractLastNumber(appointment.id)}");
    ...
    if (await utilities.isConnectionAvailable()) {
      ...
      if (response.statusCode == 200) {
        ...
        if (response.statusCode == 201) {
          ...
          utilities.error('Il y a une erreur reseau.');
        }
      }
    } else {
      utilities.handleConnectionError(
  ...
  Future<bool> sendNotificationDisableAppointment(
      {required String doctorName, required String recipientToken}) async {
    ...
    final notificationData = {
      'message': {
        ...
        'notification': {
          ...
          'body': 'Votre rendez-vous avec le Dr.$doctorName a été annulé.'
        }
      },
    };
